locals {
  role          = "docker"
  instance_name = "${var.environment}-docker"
}

resource "aws_security_group" "docker" {
  name        = local.instance_name
  description = "Allow inbound docker traffic and mgmt ssh"
  vpc_id      = var.vpc_id
...
data "template_file" "cloud_config_docker" {
...
module "asg" {
  ...
  launch_template_description = "docker launch template"
  ...
  user_data = base64encode(data.template_file.cloud_config_docker.rendered)
...
resource "aws_route53_record" "eliza" {
  zone_id = var.route53_zone_id
  name    = "${var.environment}-eliza.${var.domain}"
  type    = "A"
  alias {
    name                   = module.alb.lb_dns_name
    zone_id                = module.alb.lb_zone_id
