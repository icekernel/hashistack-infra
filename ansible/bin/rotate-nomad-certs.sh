...
nomad tls cert create -ca=$ENV-ca.pem -key=$ENV-ca-key.pem -server -region=global
mv global-server-nomad-key.pem $ENV-server-key.pem
mv global-server-nomad.pem $ENV-server.pem

nomad tls cert create -ca=$ENV-ca.pem -key=$ENV-ca-key.pem -client
mv global-client-nomad-key.pem $ENV-client-key.pem
mv global-client-nomad.pem $ENV-client.pem

nomad tls cert create -ca=$ENV-ca.pem -key=$ENV-ca-key.pem -cli
